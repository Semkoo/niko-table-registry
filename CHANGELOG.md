...
- **`enableGrouping` defaults to `false`** unless `DataTableColumnGroupOptions` / `Menu` is mounted or `config.enableGrouping` is set (previously defaulted `true` without wiring `getGroupedRowModel`). Additive for apps that never grouped; set `config.enableGrouping: true` if you relied on the old default for column `getCanGroup()` without the new UI.
