...
- **`DataTableGroupedRows`** — group rows are now composed, not configured. Nesting `<DataTableGroupedRows />` in a `DataTableBody` renders the bands **and** is what feature detection reads to enable grouping + expanding, so there is no `config.enableGrouping` to set and no renderer to wire — the same marker contract as `DataTableColumnResize` and `DataTableRowContextMenuSlot`. Bring your own band by nesting a component that reads `useDataTableGroupRow()` (no render prop, no callback); `enabledFor` falls individual groups back to the built-in row, and `DataTableGroupedRowCells` is exported for reuse. Group bands render in the standard body only — the virtualized and DnD bodies are a documented limitation. [Grouping Table](/examples/grouping-table/)
