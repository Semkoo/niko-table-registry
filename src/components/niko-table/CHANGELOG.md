...
- **`trigger` on both view menus** (`filters/table-view-menu`, `filters/table-view-dnd-menu`) — replaces the default toolbar button with any element, matching the `trigger` contract `TableColumnActions`, `TableDateFilter` and `TableFacetedFilter` already expose. Lets a consumer mount the menu as a header-sized bare icon beside a column title, or any other shape, without the component growing a boolean per shape. Defaults to the existing button, so nothing changes for callers that omit it.
...
- **Grouping trigger is reachable but invisible on keyboard focus** (`filters/table-column-group`) — the trigger is `opacity-0` until hover while remaining focusable, so a keyboard user could tab to a control they cannot see. Adds `focus-visible:opacity-100` alongside the existing `group-hover:opacity-100`.

- **Strict-index guards in virtualized DnD, filter menu, pagination and format** — under `noUncheckedIndexedAccess` these array-index reads are typed `T | undefined`, and several can be undefined at runtime. `virtualItems[0].start` and `virtualItems[virtualItems.length - 1]` in both virtualized DnD structures sit behind a `hasVirtualItems` boolean the compiler cannot connect to them, and an empty virtual range throws on the first read; `table-filter-menu`'s `previousFilter` and its two `originalFilters[...]` lookups use indices that are `-1` when a filter is not found by id or key; `table-pagination` defaults `defaultPageSize` from `pageSizeOptions[0]`, which is `undefined` for an empty options array where `setPageSize` expects a number; `lib/format` reads `keys[0]` after a length check. Behaviour is unchanged for every non-empty input — the already-intended fallbacks are now explicit.
