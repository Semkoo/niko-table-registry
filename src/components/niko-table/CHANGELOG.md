...
- **Column sizing persistence** (`lib/use-column-sizing-persistence.ts`) — opt-in `useColumnSizingPersistence(storageKey)` hook. Returns `columnSizing` + `onColumnSizingChange` (+ `resetColumnSizing`) to wire into `<DataTableRoot>` so resized widths persist to `localStorage`. Writes immediately (resize is `onEnd`, one write per drag, so nothing is lost on a hard refresh), SSR-safe, and sanitizes corrupt/hand-edited storage before it reaches `getSize()`.
