...
- We use **Conventional Commits** for commit messages; the changelog is generated from them (see [Changelog](#changelog) below).
...
We use **Conventional Commits** (`feat:`, `fix:`, `docs:`, etc.) for commit messages. Commit messages are validated via commitlint on commit.

When cutting a release, run `npm run changelog` to regenerate `src/components/niko-table/CHANGELOG.md` from git tags and commit history. The docs [Changelog](/changelog/) page displays this file automatically.
