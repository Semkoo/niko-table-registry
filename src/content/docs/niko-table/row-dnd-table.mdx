...
<CliCommandCode action="run" command="shadcn@latest add table button" />

2. Add dependencies:

<CliCommandCode action="install" command="@tanstack/react-table @dnd-kit/core @dnd-kit/modifiers @dnd-kit/sortable @dnd-kit/utilities lucide-react" />
...
5. **Install add-ons for this example** — This example uses row drag-and-drop. Run:

<CliCommandCode action="run" command={'shadcn@latest add "https://niko-table.com/r/data-table-row-dnd.json"'} />
...
- [Column DnD Table](/niko-table/column-dnd-table) — Drag-and-drop column reordering
- [Row Selection Table](/niko-table/row-selection-table) — Combine with row selection
