...
[Browse Examples](/niko-table/simple-table).
...
1. **Run the following command to install the required Shadcn components**
   ...
   <CliCommandCode action="run" command="shadcn@latest add table input button dropdown-menu popover command badge scroll-area separator checkbox select skeleton tooltip" />

2. **Add the TanStack Table dependency**

   <CliCommandCode action="install" command="@tanstack/react-table" />

3. **Update the Shadcn table component**

   Replace the contents of `components/ui/table.tsx` with the DataTable-compatible version (adds `TableComponent` export and `data-slot` attributes). See the [Installation Guide](/getting-started/installation#update-shadcn-table-component-first) for the full code.

4. **Install the DataTable core**

   From your project root, run:

   <CliCommandCode action="run" command={'shadcn@latest add "https://niko-table.com/r/data-table.json"'} />

   For optional add-ons (pagination, filters, virtualization, etc.) and manual copy-paste, see the [Installation Guide](/getting-started/installation).
...
5. **You've created your first table!**
   ...
   Your table now has search functionality and pagination. See the [Examples](/niko-table/simple-table) for more advanced configurations.
...
- [Simple Table](/niko-table/simple-table) - Basic table example
- [Basic Table](/niko-table/basic-table) - Table with pagination and sorting
- [Advanced Table](/niko-table/advanced-table) - Complex filtering and sorting
- [Server-Side Table](/niko-table/server-side-table) - Server-side data handling
