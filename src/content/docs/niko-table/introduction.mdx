...
<div className="not-content mb-6 flex items-center gap-2">
  <DocsLink href="https://tanstack.com/table/latest/docs/framework/react/overview" external>Docs ↗</DocsLink>
  <DocsLink href="https://tanstack.com/table/latest/docs/framework/react/guide/migrating" external>Migrating to v9 ↗</DocsLink>
</div>

<figure className="flex flex-col gap-4">
  <CodePreview
    demo="niko-table/all-features-table"
    title="DataTable"
    type="block"
...
[Browse Examples](/examples/simple-table/).
...
2. **Install a recommended starter set**
   ...
   Need more (virtualization, DnD, Data Grid)? See the [Installation Guide](/getting-started/installation/) or [Components](/getting-started/components/).

</Steps>

## Table Features (TanStack Table v9)

This registry targets **TanStack Table v9**. In v8, every behavior was bundled into `useReactTable`. In v9 you import only what you need via `tableFeatures()` — the same **Composable** idea as installing only the registry pieces you mount.

`DataTableRoot` wires this for you. Installing `@niko-table/data-table` (or `data-table-core`) copies `lib/data-table-features.ts` into your project. That file is the engine-layer trim point: delete features, row models, and built-in `sortFns` / `aggregationFns` your tables never use.

```tsx showLineNumbers title="components/niko-table/lib/data-table-features.ts"
import {
...
export const features = tableFeatures({
  ...
  // filterFns / sortFns / aggregationFns you actually use
})
...
- There are no more `get*RowModel` table options. Row models for optional features are registered on the features object.
- `DataTableRoot` still uses feature detection / `config.enable*` to decide _when_ behavior runs. Registration decides _whether the API exists_.
- When you own the table instance yourself, pass `features` into `useTable({ features, data, columns })` and wrap with `DataTableProvider`. See [Core](/niko-table/overview/core/).
- Full reference: [Library → Table Features](/niko-table/overview/lib/#table-features). TanStack's [migration guide](https://tanstack.com/table/latest/docs/framework/react/guide/migrating).

### Upgrading an existing install

Copied v8 code keeps working until you bump the dependency and re-add the components. Expect:

| v8                                       | v9                                                                                           |
| ---------------------------------------- | -------------------------------------------------------------------------------------------- |
| `useReactTable` + `get*RowModel` options | `useTable` + `features`                                                                      |
| `table.getState().x`                     | `table.state.x`                                                                              |
| `VisibilityState`                        | `ColumnVisibilityState`                                                                      |
| `columnPinning: { left, right }`         | `columnPinning: { start, end }`                                                              |
| `createColumnHelper<TData>()`            | `createDataTableColumnHelper<TData>()` (or `createColumnHelper<DataTableFeatures, TData>()`) |
...
5. **You've created your first table!**
   ...
   Your table now has search functionality and pagination. See the [Examples](/examples/simple-table/) for more advanced configurations.
...
See the [Components](/niko-table/overview/components/) page for detailed documentation on each component.
...
| `table`                    | `DataTableInstance<TData>`                          | Pre-configured TanStack Table instance (optional). |
...
| `onColumnVisibilityChange` | `(updater: Updater<ColumnVisibilityState>) => void` | Callback when column visibility changes.           |
...
| `enableGrouping`     | `boolean` | `false` | Enable column grouping (not tree `getSubRows`). Auto-enabled from column group UI. |
...
- [Components](/niko-table/overview/components/) - Detailed component documentation
- [Simple Table](/examples/simple-table/) - Basic table example
- [Basic Table](/examples/basic-table/) - Table with pagination and sorting
- [Advanced Table](/examples/advanced-table/) - Complex filtering and sorting
- [Server-Side Table](/examples/server-side-table/) - Server-side data handling
...
### [Core Components](/niko-table/overview/core/)
...
### [User-Facing Components](/niko-table/overview/components/)
...
### [Filter Components](/niko-table/overview/filters/)
...
### [Hooks](/niko-table/overview/hooks/)
...
- **[Shadcn UI](https://ui.shadcn.com)** by Shadcn - Beautiful, accessible component primitives built on Radix UI. All UI components in Niko Table are built using Shadcn UI components.
