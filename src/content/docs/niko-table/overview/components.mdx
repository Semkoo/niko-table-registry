...
Opt-in column resizing. Drop `<DataTableColumnResize />` inside `DataTableRoot` to enable drag grips (and keyboard nudge / double-click autosize) on every header that allows resize. Works with both regular and virtualized headers. Opt out per column with `enableResizing: false`. See the [Column Resize Table](/examples/column-resize-table/) example.
