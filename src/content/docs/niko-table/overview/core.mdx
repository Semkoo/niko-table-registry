...
The `DataTableRoot` component provides the table context to all child components. You should always wrap your table in a `DataTableRoot`. It initializes the TanStack Table instance with `useTable` and the shared `features` object from `lib/data-table-features.ts` (TanStack Table v9), and manages table state.
...
| `onColumnVisibilityChange` | `(updater: Updater<ColumnVisibilityState>) => void` | Called when column visibility changes. |
...
| `enableGrouping`       | `boolean` | `false` | Enable column grouping. Auto-enabled when column group UI is detected. Do not combine with tree `getSubRows`. |
...
Low-level context provider used internally by `DataTableRoot`. Use this directly when you need to provide a pre-configured TanStack Table instance to the context (e.g., when using `useTable` yourself).
