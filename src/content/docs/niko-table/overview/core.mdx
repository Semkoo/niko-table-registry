...
> **Row memo invalidation:** all six body components are wrapped in `React.memo` for performance. When you rebuild the `columns` array via `useMemo([externalState, ...])`, every row invalidates automatically (via the internal `useColumnDefsVersion` hook keyed on `columnDef` references). Use `getRowMemoKey` only when you want **per-row** invalidation (e.g. only the row being edited re-renders).
