...
<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/constants.ts`} external>Source Code ↗</DocsLink>
</div>
...
<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/data-table.ts`} external>Source Code ↗</DocsLink>
</div>
...
<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/format.ts`} external>Source Code ↗</DocsLink>
</div>
...
In Table v8, every behavior was bundled into `useReactTable`. In Table v9 you import only what you need — the same idea as this registry's **Composable** principle: mix and match `DataTable*` components, install only the registry pieces you need. Registry items are that idea at the UI layer; this file is that idea at the table-engine layer.

<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/data-table-features.ts`} external>Source Code ↗</DocsLink>
</div>

A feature's API only exists on the table instance once it is registered. `DataTableRoot` passes this object to `useTable` for you. Feature detection / `enable*` flags still decide _when_ behavior runs; registration decides _whether the API exists at all_.
...
// DataTableRoot does this internally — you only need it for a custom table.
const table = useTable({ features, data, columns })
```

`DataTableFeatures` is the type of that object, and every `DataTable*` type alias is pre-bound to it (`DataTableInstance<TData>` is `Table<DataTableFeatures, TData>`, and so on), so you rarely name it directly.

### createDataTableColumnHelper

Pre-bound TanStack Table v9 column helper. TanStack's docs recommend `createColumnHelper()` first for static columns (typed `getValue()` in cells); the raw v9 helper needs the features generic hand-threaded (`createColumnHelper<DataTableFeatures, TData>()`). This wraps it the same way `DataTableColumnDef<TData>` pre-binds features.
...
const columns = [
  ...
  columnHelper.accessor("price", {
    header: "Price",
    cell: ({ getValue }) => getValue().toFixed(2), // `getValue()` is `number`
  }),
...
Use the helper when you want typed `getValue()` in cells. Helper results belong in a `DataTableColumns<TData>` array (what `DataTableRoot` accepts). Use a plain `DataTableColumnDef<TData>[]` for dynamic columns and maximum copy-paste-ability — that is still the default in examples.

#### Trimming it down

The shipped file registers everything the component library can use — sorting, filtering, faceting, grouping, aggregation, pagination, expanding, selection, column visibility/ordering/pinning/sizing/resizing — plus the row models they need. After install you own the file: delete the features and the built-in `sortFns` / `aggregationFns` your tables never reference, the same way you omit registry items you never mount. Removing a feature removes its API: calling `table.setGrouping()` without `columnGroupingFeature` fails at runtime, not at build time.

#### Adding your own filter function

Register it under `filterFns` and reference it by name from a column:
...
export const features = tableFeatures({
  ...
  filterFns: {
    ...
    startsWith: (row, columnId, value: string) =>
      String(row.getValue(columnId)).toLowerCase().startsWith(value.toLowerCase()),
  },
})
...
<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/filter-functions.ts`} external>Source Code ↗</DocsLink>
</div>
...
<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/styles.ts`} external>Source Code ↗</DocsLink>
</div>
...
| Name       | Type                     | Default | Description                                                                          |
| ---------- | ------------------------ | ------- | ------------------------------------------------------------------------------------ |
| `column`   | `DataTableColumn<TData>` | -       | TanStack Table column instance                                                       |
...
<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/create-scroll-handler.ts`} external>Source Code ↗</DocsLink>
</div>
...
- For _virtualized_ infinite scroll, prefer `DataTableVirtualizedBody`'s `onNearEnd` + `prefetchThreshold` props over `onScrolledBottom` — `onNearEnd` is virtualizer-index-driven (catches fast scrolls, scrollbar drag, programmatic `scrollToIndex()` jumps, and initial renders where the dataset doesn't fill the viewport).
...
<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/row-click.ts`} external>Source Code ↗</DocsLink>
</div>
...
<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/build-faceted-options.ts`} external>Source Code ↗</DocsLink>
</div>
...
Internal helpers that resolve `globalFilter` / column-filter combinations to a final visible row set. Used by `DataTableRoot` on top of the filtered row model registered in `lib/data-table-features.ts`.

<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/filter-rows.ts`} external>Source Code ↗</DocsLink>
</div>
...
This module is internal to the table's filter pipeline. Consumers normally interact with it indirectly through the `filterFns` registered in `tableFeatures()` (see `lib/data-table-features.ts`), which `DataTableRoot` passes to the table automatically. If you're building a fully custom filter pipeline you can import the named helpers from this file directly — see the source for the current export surface.

## Column Sizing Layout

Helpers that keep a resizable table filling its container and its header labels legible. Flex fill and header-fit are on automatically once column resizing is enabled; the auto-fit and persistence helpers are opt-in.

### resolveFlexColumnIds

Resolves which column(s) render widthless so they absorb the leftover row width (flex fill). Flex fill is **on by default** for resizable tables: the first non-pinned, resizable data column soaks up the surplus so the table fills its container instead of leaving dead space on the right. Pure layout — it never writes `columnSizing`, so there are no persistence side effects.

Overrides via column / table `meta`: `meta.flex === true` flexes that specific column, `meta.flex === false` opts a column out, and `TableMeta.disableFlexFill === true` turns fill off for the whole table. A column the user has explicitly resized is fixed at their width and hands the fill to the next eligible column.

<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/flex-columns.ts`} external>Source Code ↗</DocsLink>
</div>

```tsx showLineNumbers
import { resolveFlexColumnIds } from "@/components/niko-table/lib/flex-columns"

const flexIds = resolveFlexColumnIds(table)
// → Set { "name" } — the default first data column fills the surplus
```
...
| Name    | Type                       | Description             |
| ------- | -------------------------- | ----------------------- |
| `table` | `DataTableInstance<TData>` | TanStack Table instance |

#### Returns

`ReadonlySet<string>` - Column ids to render widthless. Empty when fill is off (`disableFlexFill`, resizing disabled) or no eligible column exists.

### resolveColumnWidth

The width to render for a single column. Centralizes the flex + header-fit rules so the header, body cells, and the min-width lock all agree. Returns the declared `size` when resizing is off, `undefined` for the flex column, exactly `getSize()` for a user-resized column, and otherwise floors the width at the header's natural width so the label never truncates on load. Pure layout — `headerMinWidths` is measured, never written back into `columnSizing`.
...
```tsx showLineNumbers
import { resolveColumnWidth } from "@/components/niko-table/lib/flex-columns"

const width = resolveColumnWidth(column, {
  resizing: true,
  isFlex: flexIds.has(column.id),
...
| ------------------------ | ----------------------------- | -------------------------------------------------- |
| `column`                 | `DataTableColumn<TData>`      | TanStack Table column instance                     |
| `params.resizing`        | `boolean`                     | Whether column resizing is enabled                 |
| `params.isFlex`          | `boolean`                     | Whether this column is a flex (fill) column        |
| `params.columnSizing`    | `ColumnSizingState`           | Current column sizing state                        |
| `params.headerMinWidths` | `ReadonlyMap<string, number>` | Measured header widths (from `useHeaderMinWidths`) |

#### Returns

`number | string | undefined` - The width to render (`undefined` for the flex column or an unsized off state).

### useHeaderMinWidths

Measures each column header's natural width off-DOM (a reusable canvas, so no reflow) and floors un-resized columns at their header width — header-fit — so labels don't truncate on load. Returns a `columnId → minWidth` map; a column the user has explicitly resized is left alone. Pure measurement, nothing is written to `columnSizing`. Recomputes only when the visible columns, their labels, or the header font change.

<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/use-header-min-widths.ts`} external>Source Code ↗</DocsLink>
</div>

```tsx showLineNumbers
import { useHeaderMinWidths } from "@/components/niko-table/lib/use-header-min-widths"

const headerMinWidths = useHeaderMinWidths(table, scrollElement, resizingEnabled)
```
...
| --------------- | -------------------------- | ------------------------------------------------ |
| `table`         | `DataTableInstance<TData>` | TanStack Table instance                          |
| `scrollElement` | `HTMLElement \| null`      | The table's scroll container (for reading fonts) |
| `enabled`       | `boolean`                  | Whether header-fit measurement is active         |

#### Returns

`ReadonlyMap<string, number>` - Minimum width in px per column id (label width + header chrome).

### useColumnAutoFit

The even-split alternative to single-column flex fill. On load (and when the container grows or columns toggle) it grows the resizable columns by an equal share of the leftover space so they fill the available width, seeding `columnSizing`. Backs off permanently once the user resizes a column or a saved `columnSizing` is restored. Consumed by the opt-in [`<DataTableColumnAutoFit />`](/niko-table/overview/components/#datatablecolumnautofit) marker — most consumers drop that component rather than call this hook directly.

<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/use-column-auto-fit.ts`} external>Source Code ↗</DocsLink>
</div>

```tsx showLineNumbers
import { useColumnAutoFit } from "@/components/niko-table/lib/use-column-auto-fit"

useColumnAutoFit(table, scrollElement, resizingEnabled)
```
...
| --------------- | -------------------------- | ------------------------------------------------- |
| `table`         | `DataTableInstance<TData>` | TanStack Table instance                           |
| `scrollElement` | `HTMLElement \| null`      | The table's scroll container (measured for width) |
| `enabled`       | `boolean`                  | Whether auto-fit is active (needs resizing on)    |

#### Returns

`void` - Side-effecting hook; seeds `columnSizing` via `table.setColumnSizing`.

### useColumnSizingPersistence

Opt-in `localStorage` persistence for resized column widths. Column sizing is controlled state on `<DataTableRoot>`, so this hook owns it for you: wire the returned `columnSizing` + `onColumnSizingChange` into the table and widths survive reloads. Writes immediately (resizing runs in `onEnd` mode, so once per drag), syncs across tabs via the native `storage` event (an unchanged read keeps state identity, so idle tabs don't re-render), and sanitizes corrupt / hand-edited storage so a bad value never reaches `column.getSize()`.

<div className="not-content mb-4 flex items-center gap-2">
  <DocsLink href={`${GITHUB_REPO_URL}/blob/main/src/components/niko-table/lib/use-column-sizing-persistence.ts`} external>Source Code ↗</DocsLink>
</div>
...
| Name         | Type     | Description                                                   |
| ------------ | -------- | ------------------------------------------------------------- |
| `storageKey` | `string` | `localStorage` key. Scope it per table (and per tenant/user). |

#### Returns

`ColumnSizingPersistence` — an object with:

| Property               | Type                            | Description                                                       |
| ---------------------- | ------------------------------- | ----------------------------------------------------------------- |
| `columnSizing`         | `ColumnSizingState`             | Controlled widths to pass to `<DataTableRoot state={{ … }} />`    |
| `onColumnSizingChange` | `OnChangeFn<ColumnSizingState>` | Pass to `<DataTableRoot onColumnSizingChange={…} />`              |
| `resetColumnSizing`    | `() => void`                    | Clears stored widths and resets every column to its declared size |
