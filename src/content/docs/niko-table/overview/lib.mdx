...
The even-split alternative to single-column flex fill. On load (and when the container grows or columns toggle) it grows the resizable columns by an equal share of the leftover space so they fill the available width, seeding `columnSizing`. Backs off permanently once the user resizes a column or a saved `columnSizing` is restored. Consumed by the opt-in [`<DataTableColumnAutoFit />`](/getting-started/components/#column-header-controls) marker — most consumers drop that component rather than call this hook directly.
