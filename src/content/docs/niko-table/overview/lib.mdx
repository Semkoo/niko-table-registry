...
The single place where Niko Table declares which TanStack Table v9 features, row models, and filter/sort/aggregation functions its components may use.
...
TanStack Table v9 no longer bundles every behavior into the core. A feature's API only exists on the table instance once the feature is registered, so `DataTableRoot` passes this object to `useTable` for you.
...
The file registers everything the component library can use — sorting, filtering, faceting, grouping, aggregation, pagination, expanding, selection, column visibility/ordering/pinning/sizing/resizing — plus the row models they need. Since you own the file, delete the features and the built-in `sortFns` / `aggregationFns` your tables never reference to shrink your bundle. Removing a feature removes its API: calling `table.setGrouping()` without `columnGroupingFeature` will fail at runtime, not at build time.
