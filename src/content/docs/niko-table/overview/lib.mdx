...
Opt-in `localStorage` persistence for resized column widths. Column sizing is controlled state on `<DataTableRoot>`, so this hook owns it for you: wire the returned `columnSizing` + `onColumnSizingChange` into the table and widths survive reloads. Writes immediately (resizing runs in `onEnd` mode, so once per drag), is SSR-safe, and sanitizes corrupt / hand-edited storage so a bad value never reaches `column.getSize()`.
