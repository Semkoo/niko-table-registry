...
`NikoTableFeatures` is the type of that object, and every Niko type alias is pre-bound to it (`DataTableInstance<TData>` is `Table<NikoTableFeatures, TData>`, and so on), so you rarely name it directly.
