...
Data table row type. Alias for TanStack Table's `Row`, pre-bound to Niko's feature set.

```tsx
type DataTableRow<TData> = Row<NikoTableFeatures, TData>
```

### DataTableInstance

Data table instance type. Alias for TanStack Table's `Table`, pre-bound to Niko's feature set.

```tsx
type DataTableInstance<TData> = Table<NikoTableFeatures, TData>
```
