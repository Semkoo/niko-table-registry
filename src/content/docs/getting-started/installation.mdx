...
Want all components at once? The following command installs **core + all optional add-ons** in one go. It uses the `@niko-table/` namespace, so the niko-table registry must be configured in your `components.json`.
