...
<Steps>
  1. **Install the required dependencies**
     ...
     You also need `lib/data-table-features.ts` from the files below — that is the `tableFeatures()` registration `DataTableRoot` passes to `useTable`. See [Table Features](/niko-table/introduction/#table-features-tanstack-table-v9).
...
│   │       │   ├── constants.ts
│   │       │   ├── data-table.ts
│   │       │   ├── data-table-features.ts                   # TanStack Table v9 tableFeatures() + createDataTableColumnHelper
│   │       │   ├── filter-functions.ts
│   │       │   ├── filter-rows.ts
...
│   │       │   ├── render-cell-content.tsx                      # Grouped / aggregated / placeholder cell renderer
