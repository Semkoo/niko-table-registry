...
- Group bands render in the standard `DataTableBody`. The virtualized and DnD bodies do not render them — a grouped row model there still groups the data, but you get leaf rows with no band. Documented limitation, not an oversight: a band is a different height from a leaf row, which the virtualizer's size estimate has to account for.
