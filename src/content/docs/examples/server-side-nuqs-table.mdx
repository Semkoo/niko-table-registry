...
Server-side tables can't rely on TanStack's client-side faceted unique values — those only see the current page. Instead, the server returns per-column cross-filter facets and the client merges them into the static option list.

### How it works

For each select column, the server computes the option list **excluding that column's own filter** and applying every other active filter. This produces:

- For the column being filtered: still includes every value → the user can pivot to any other option without losing their filter set.
- For other columns: only includes values that would still produce non-empty results → impossible options (e.g. `Brand: Nike` while `Category: Electronics` is active) are pruned.
...
// Per column, re-filter excluding its own filter
...
for (const col of selectColumns) {
  ...
  facets.select[col] = [...counts.entries()].map(([value, count]) => ({
    value,
    count,
  }))
}
...
Just merge counts onto your static label list — the faceted filter hides options with `count === 0` by default, so cross-filter narrowing happens automatically:
...
// Pass as `options` to <DataTableColumnFacetedFilterMenu />
```

The `count: 0` rule is a default — pass `count: undefined` on options you want visible regardless. Pure label-only callers (no counts at all) are unaffected; the filter falls back to client-derived row counts.
...
- [Advanced Table](/examples/advanced-table/) - Learn about advanced filtering
