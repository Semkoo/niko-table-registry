...
Try it: filter or sort the demo, watch the "URL State" card update, and imagine pasting that URL to a teammate. The demo ships with a mock server so every filter works out of the box; everything in its `MOCK SERVER` section is a stand-in for your backend.
