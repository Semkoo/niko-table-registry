...
The demo above ships with a **mock server** (an in-memory array plus latency) so every filter works out of the box. Everything in its clearly-marked `MOCK SERVER` section is a stand-in for your backend.
