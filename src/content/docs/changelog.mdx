...
- **Column auto-fit** — resizable columns fill the container on load instead of leaving dead space (`useColumnAutoFit`); fixed columns keep their size, and restored/persisted widths win. [example](/examples/column-resize-table/)
