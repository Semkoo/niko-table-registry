...
- **Column sizing persistence** — opt-in `useColumnSizingPersistence(storageKey)` hook: wire its `columnSizing` + `onColumnSizingChange` into `<DataTableRoot>` and resized widths persist to `localStorage` (immediate write, SSR-safe). [Persist Widths](/examples/column-resize-table/#persist-widths)
