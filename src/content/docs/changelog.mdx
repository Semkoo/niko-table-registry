...
- **Faceted filter, filter menu, inline filter — options with `count: 0` are hidden by default.** Cross-filter narrowing is now consistent across every filter UI: column-header faceted filter, `<DataTableFilterMenu />` builder, and inline filter all hide options whose count resolved to 0. Pure label-only callers (no counts anywhere) are unaffected. Opt-out per option by passing `count: undefined`.
