...
- **`createDataTableColumnHelper`** — pre-bound TanStack Table v9 column helper in `lib/data-table-features.ts`. Typed `getValue()` in cells without hand-threading `DataTableFeatures`. Plain `DataTableColumnDef<TData>[]` stays the default in examples. [Lib](/niko-table/overview/lib/)
- **`data-table-ui`** — optional `components/ui/table.tsx` (`TableComponent` + `tabIndex={0}` scroll container). Pulled by `data-table`; skipped by `data-table-core` so customized tables survive controls-only installs. Part of [#180](https://github.com/Semkoo/niko-table-registry/issues/180)
- **`data-table-core`** — shared floor (context, structure, types) split out of `data-table`. À-la-carte controls depend on it so server-driven installs skip `DataTableRoot`. [Components](/getting-started/components/#core) · closes [#180](https://github.com/Semkoo/niko-table-registry/issues/180)
- **Row grouping / aggregation** — `@niko-table/data-table-column-group` with **Group by** / **Stop grouping by** in column menus; TanStack aggregations and `getGroupingValue` (e.g. group by month). [Grouping Table](/examples/grouping-table/)
- **Base UI support** — all registry items install cleanly into both shadcn generations: Radix (`new-york`) and Base UI (`base-nova`, the current `shadcn init` default). Dual-generation-safe callbacks and props; verified with fresh-app installs of every block under both styles.
...
- **Drizzle ORM + Nuqs** — the same Drizzle SQL layer with table state in the URL via nuqs (shareable, bookmarkable, refresh-proof). [guide](/examples/drizzle-orm-nuqs/)
...
- **Column auto-fit** — opt-in `<DataTableColumnAutoFit />` marker that spreads leftover space evenly across resizable columns (the alternative to single-column fill); fixed columns keep their size, restored/persisted widths win. [example](/examples/column-resize-table/)
- **Column sizing persistence** — opt-in `useColumnSizingPersistence(storageKey)` hook: wire its `columnSizing` + `onColumnSizingChange` into `<DataTableRoot>` and resized widths persist to `localStorage` (immediate write, cross-tab sync). [Persist Widths](/examples/column-resize-table/#persist-widths)
...
- Docs — [Documentation Guidelines](/contributing/documentation-guidelines/)
- Registry — `data-table-virtualized-row-dnd` / `data-table-virtualized-column-dnd`

### Changed

- **TanStack Table v9** — the registry now targets `@tanstack/react-table@^9`. Tables are built with `useTable` and a shared `lib/data-table-features.ts` that declares which features and row models are available; `DataTableRoot` wires it for you. For column authoring, `createDataTableColumnHelper<TData>()` pre-binds the features generic. v8 bundled every behavior into `useReactTable`; v9 imports only what you need — the same **Composable** principle as installing only the registry pieces you need, at the table-engine layer. There is no parallel v8 docs track — code you already copied keeps working on v8 until you bump the dependency and re-add the components. Renames to expect in your own code: `VisibilityState` → `ColumnVisibilityState`, `table.getState()` → `table.state`, and column pinning uses logical `start` / `end` instead of `left` / `right`. Two clean breaks: persisted v8 `columnPinning` state (localStorage, shared URLs, server prefs) still carries `left`/`right` keys, which v9 ignores — pinning resets on first load after the upgrade (re-pin and re-save; no runtime migration). And dynamic facet counts resolve each column's filter through `column.getFilterFn()` — the same callable the table filters with — so externally-built tables passed to `Table*` components may see option counts shift where v8 skipped unregistered built-in filter names. See TanStack's [migration guide](https://tanstack.com/table/latest/docs/framework/react/guide/migrating) · closes [#177](https://github.com/Semkoo/niko-table-registry/issues/177)
...
- [Installation](/getting-started/installation/) notes both shadcn generations are supported (`new-york` Radix and `base-nova` Base UI)
...
- **Core** — `data-row-index` is display order (`flashRows` / scroll after sort/filter)
- **Core** — empty/loading `colSpan` uses visible columns; resize width cleanup restores prior table styles
- **Core** — `daysAgo()` is deterministic (fixed epoch)
