...
- **Persistence** (`data-table-grid-changes`) — `useGridChanges` → create / update / delete. [Persistence](/data-grid/persistence/)
- **Inline validation** — `CellState.error` tooltip on the cell. [Validation](/data-grid/validation/)
- **Column resize** — `<DataTableColumnResize />`; [example](/examples/column-resize-table/)
- Docs pass — shorter, shadcn-style install → usage across the Data Grid guides

### Fixed

- **Core** — `data-row-index` is display order (`flashRows` / scroll after sort/filter)
- **Core** — empty/loading `colSpan` uses visible columns; resize width cleanup
