...
For a walkthrough, start with the [Introduction](/data-grid/introduction/).
...
`updateRows(fn, { history: false })` settles derived state (e.g. cross-field validation) without pushing undo. See [Validation](/data-grid/validation/).
...
`useGridColumns({ initialColumns })` + `<DataGridColumns value={cols}>` + `<GridColumnMenuOptions />` + `<DataGridAddColumnButton />`. See [Dynamic Columns](/data-grid/dynamic-columns/).
...
Driven by `grid.lastCommit` (`seq` advances on real commits only). See [Persistence](/data-grid/persistence/).
